...
def fetch_nba_injuries():
    ...
    try:
        ...
        if not table:
            raise ValueError("Injury table not found on the page")
...
if injuries:
    st.markdown("## 🩹 NBA Injury Report (via RotoWire)")
    for team, players in injuries.items():
        with st.expander(f"📋 {team} - {len(players)} player(s) listed"):
            for player in players:
                st.markdown(
                    f"**{player['name']}** ({player['position']}) — *{player['status']}*<br>{player['injury']}",
                    unsafe_allow_html=True
                )

else:
    st.markdown("✅ No injuries reported.")
