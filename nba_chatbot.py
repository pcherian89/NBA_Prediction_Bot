...
def fetch_nba_injuries_rotowire():
    ...
    try:
        ...
        # ✅ Adjusted class name and selector based on page structure
        table = soup.select_one("table.tablesorter")

        if not table:
            raise ValueError("Could not locate injury table on page.")
...
injuries = fetch_nba_injuries_rotowire()
if injuries:
    st.subheader("🩺 Injury Report")
    for player in injuries:
        st.markdown(f"- **{player['name']}** ({player['team']}, {player['position']}): {player['injury']} — *{player['status']}*")
else:
