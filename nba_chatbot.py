...
class GamePredictionAgent:
    ...
    def get_player_data(self, players: List[Dict[str, str]], team_name: str) -> pd.DataFrame:
        ...
        for p in players:
            conditions.append("(firstname ILIKE %s AND lastname ILIKE %s)")
            params.extend([p['first'], p['last']])

        where_clause = " OR ".join(conditions)

        query = f"""
            SELECT * FROM player_stats_filtered
            WHERE playerteamname ILIKE %s AND ({where_clause})
            ORDER BY gamedate DESC
        """

        # Team name should be the FIRST parameter
        full_params = [team_name] + params

        return pd.read_sql(query, engine, params=full_params)
