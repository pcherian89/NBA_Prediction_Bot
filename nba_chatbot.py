...
def fetch_injuries_from_sportradar(api_key):
    url = f"https://api.sportradar.com/nba/trial/v5/en/league/injuries.json?api_key={g2GICSQ9tHZpX2PYjKk2gNlPHr6ZqINPNi3t4OVV}"
    try:
        ...
        for team in data.get("teams", []):
            ...
            for player in team.get("players", []):
                injuries.append({
                    ...
                    "injury": player.get("injuries", [{}])[0].get("description", "N/A"),
...
with st.expander("🧠 Click to View Today's Injury Report"):
    injuries = fetch_injuries_from_sportradar(g2GICSQ9tHZpX2PYjKk2gNlPHr6ZqINPNi3t4OVV)
    if injuries:
        for p in injuries:
            st.markdown(f"**{p['name']}** ({p['position']} - {p['team']}) — *{p['injury']}*, **{p['status']}**")
    else:
        st.info("✅ No injuries reported or data could not be loaded.")
