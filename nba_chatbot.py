    ...
    for team, players in injuries.items():
        with st.expander(f"📋 {team} - {len(players)} player(s) listed"):
            for player in players:
                st.markdown(f"**{player['name']}** — *{player['status']}*<br>{player['description']}", unsafe_allow_html=True)

else:
    st.markdown("✅ No injuries reported today.")
