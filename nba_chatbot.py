...
def get_players_by_team(team_name):
    ...
    # ✅ Clean and deduplicate player names
    df["fullname"] = df["firstname"].str.strip() + " " + df["lastname"].str.strip()
    unique_names = df["fullname"].drop_duplicates().sort_values().tolist()
...
# ✅ Prediction Agent (Assuming GamePredictionAgent is imported or defined below)
if (
    home_team != "Select a team" and away_team != "Select a team" and
    len(home_players) == 3 and len(away_players) == 3
):
    if st.button("🔮 Run Prediction"):
        from nba_agent import GamePredictionAgent  # or define the class in this file
