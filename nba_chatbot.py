...
if games_today:
    ...
    with col2:
        if st.button("Select", key=f"select_game_{idx}"):
            st.session_state.home_team = game["home_team"]
            st.session_state.away_team = game["away_team"]
...
with col1:
    selected_home = st.session_state.get("home_team", "Select a team")
    home_team = st.selectbox("🏠 Home Team", teams_list, index=teams_list.index(selected_home) if selected_home in teams_list else 0)
with col2:
    selected_away = st.session_state.get("away_team", "Select a team")
    away_team = st.selectbox("✈️ Away Team", teams_list, index=teams_list.index(selected_away) if selected_away in teams_list else 0)
